...
def load_data(data_array, labels_before,k = 2):
	data = []
	labels = []
	label_set = np.unique(labels_before)
	ii = 0
	for l in label_set:
		idx = np.where(labels_before == l)[0]
		for i in range(int(np.floor(len(idx)/k))):
			new_point = []
			for j in range(k):
				new_point.append(data_array[j+i*k,:])
			data.append(np.linalg.qr(np.vstack(new_point).T)[0])
			labels.append(ii)
		ii+=1
...
def run_grlbg(do_median, data, labels_before, Ldims = 5,psval = False, purity = False):
        ...
        if purity:
            ...
            [centers,labels_after_med] = lbg_test_obj.fit(data,center_count = 3, median = True, l_vec_dims = Ldims, plot_svals = psval, eigplot = False, distortion_plot = False)
            [centers,labels_after_mean] = lbg_test_obj.fit(data,center_count = 3, median = False, l_vec_dims = Ldims, plot_svals = psval, eigplot = False, distortion_plot = False)
            return [cluster_purity(labels_after_med,labels_before),cluster_purity(labels_after_mean,labels_before)]
        
        if psval:
            [centers,labels_after,S] = lbg_test_obj.fit(data,center_count = 3, median = do_median, l_vec_dims = Ldims, plot_svals = psval, eigplot = False, distortion_plot = False)
            print('Cluster Purity: ')
            print(cluster_purity(labels_after,labels_before))
            return centers, S
        else:
            [centers,labels_after] = lbg_test_obj.fit(data,center_count = 3, median = do_median, l_vec_dims = Ldims, plot_svals = psval, eigplot = False, distortion_plot = False)
            print('Cluster Purity: ')
            print(cluster_purity(labels_after,labels_before))
...
out_k = 2
#load the data
ccd = calcom.io.CCDataSet('/data3/darpa/all_CCD_processed_data/columbia-TCC-rnaseq.h5')
# ccd = calcom.io.CCDataSet('../data/columbia-TCC-rnaseq.h5')


[data_array, labels_raw] = ccd.generate_classification_problem('Phenotype')

[data_list, labels_before] = load_data(data_array, labels_raw)
