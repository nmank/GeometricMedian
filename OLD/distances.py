...
def geodesic(C, X, t):
    m = C.shape[0]
    np.dot(np.dot(np.eye(m) - np.dot(C, np.transpose(C)), X), np.linalg.inv(np.transpose(C), X))
    U, S, Vh = np.linalg.svd(np.dot(np.dot(np.eye(m) - np.dot(C, np.transpose(C)), X), np.linalg.inv(np.dot(np.transpose(C), X))),
                             full_matrices=False)
    Y = np.dot(C, np.dot(np.transpose(Vh), np.diag(np.cos(np.arctan(S)*t)))) + np.dot(U, np.diag(np.sin(np.arctan(S)*t)))
    Q = np.linalg.qr(Y)[0]
    return Q
...
def chordal_distance(X, Y, mdn):
    ...
    if type(X) != list or type(Y) != list:
        xdoty = np.dot(np.transpose(X), Y)
        costheta = np.linalg.svd(np.dot(np.transpose(xdoty), xdoty))[1]
        sinsquares = 1 - costheta
        ...
        for i in range(m):
            for j in range(n):
                if type(X[i]) == list:
                    ...
                    #nate changed this also
                    xdoty = np.dot(np.transpose(X), Y)
                    if len(xdoty) != 1:
                        costheta = np.linalg.svd(np.dot(np.transpose(xdoty), xdoty))[1]
                    else:
                        costheta = xdoty
                    # xTdoty = np.dot(np.transpose(X[i]), Y[j])
                    # costheta = np.linalg.svd(xTdoty)[1]
                    sinsquares = 1 - costheta
...
def geodesic_distance(X, Y):
    ...
    for i in range(m):
        for j in range(n):
            costheta = np.linalg.svd(np.dot(np.transpose(X[i]), Y[j]))[1]
